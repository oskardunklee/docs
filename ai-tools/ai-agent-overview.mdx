...
description: 'Comprehensive guide to Stanna\'s AI-powered assistant for customer success automation'
---

# Meet Lars - Your AI Assistant

Lars is Stanna's intelligent AI assistant that transforms how you manage customer success. Using natural language, Lars analyzes client health, identifies risks, creates tasks, and provides actionable insights—all in real-time.
...
Lars is available throughout the Stanna platform:

### Chat Widget
Floating AI assistant accessible from any page:
- **Location**: Bottom-right corner of every screen
- **Quick access**: Click the sparkle icon to open
- **Persistent**: Remembers your conversation across pages
- **Keyboard shortcut**: Press `⌘L` (Cmd+L) to open Lars instantly

### Client Profiles
Context-aware assistance on individual client pages:
- **Client-specific insights**: Lars automatically knows which client you're viewing
- **Smart suggestions**: Get recommendations based on that client's data
- **Quick actions**: Create tasks or send emails without leaving the profile

### Dashboard Insights
Proactive alerts and recommendations:
- **Risk notifications**: Lars highlights clients needing attention
- **Trend analysis**: Automatic insights about portfolio changes
- **Daily briefings**: Morning summary of what to prioritize
...
### Get Insights
```http
GET /api/ai-agent/insights?clientId=client-uuid&isResolved=false
X-API-Key: your-api-key
X-Workspace-Id: your-workspace-id
```
...
### AI Preferences
Users and workspaces can configure AI behavior:

```typescript
{
  "autoExecuteActions": false,     // Auto-execute low-risk actions
  "notificationLevel": "medium",   // low, medium, high
  "preferences": {
    "riskThreshold": 0.7,         // Risk assessment sensitivity
    "responseStyle": "detailed",   // brief, detailed, technical
    "actionConfirmation": true     // Require confirmation before execution
  }
}
```
...
- **Low Risk**: Generate reports, create tasks, add tags
- **Medium Risk**: Update scores, schedule meetings
- **High Risk**: Send emails, update client status, modify critical data
...
Lars categorizes actions by risk level:

| Risk Level | Examples | Requires Approval |
|------------|----------|-------------------|
| **Low** | Create tasks, generate reports, add notes | Optional auto-approve |
| **Medium** | Update health scores, schedule meetings | Always requires approval |
| **High** | Send emails, modify client status | Always requires approval + confirmation |

### Privacy and Security

<Warning>
**Data Protection**: Lars processes data securely within your workspace. No client information is shared with external AI services or third parties.
</Warning>

- **Workspace isolation**: Lars only accesses data within your workspace
...
- **No external sharing**: Client data never leaves Stanna's infrastructure
...
<CardGroup cols={2}>
  <Card title="Lars Tutorial" icon="graduation-cap" href="/ai-tools/ai-agent-tutorials">
    Step-by-step guide to using Lars effectively
  </Card>
  ...
  <Card title="API Documentation" icon="code" href="/ai-tools/ai-agent-api">
    Integrate Lars into your custom workflows
  </Card>

  <Card title="Client Management" icon="users" href="/features/client-management">
    Learn how Lars enhances client management
  </Card>
</CardGroup>